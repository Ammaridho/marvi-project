<div class="card mb-3">
    <div class="card-body">
        <div class="d-flex justify-content-between align-items-center">
            <div><h4 class="m-0">Manage Banner</h4></div>
            <div>
                <a href="javascript:void(0);" class="btn
                btn-primary text-nowrap" id="add-banner">
                    <i class="tf-icons bx bx-plus"></i> Add Banner
                </a>
            </div>
        </div>
        <hr>
        ...
        <div class="table-responsive text-nowrap pb-4 always-show" id="table-scroll">
            <table class="table table-borderless">
                ...
                <tbody id="data-banner">
                    @include('arvi.backend.marketing.banner.page-banner-child')
                </tbody>
            </table>
        </div>
...
<script>
    ...
    $('.filter').on('keyup click change',function () {
        ...
        $.get("{{ route('banner-list-table') }}",
        {search:search,active:active},function (data) {
            $('#data-banner').html(data);
        })
    })
