...
<html class="no-js" lang="en">
    <head>
        ...
        <!-- Google tag (gtag.js) -->
        <script async src="https://www.googletagmanager.com/gtag/js?id={{session('ga_code')}}"></script>
        <script>
            window.dataLayer = window.dataLayer || [];
            function gtag(){dataLayer.push(arguments);}
            gtag('js', new Date());

            gtag('config', "{{session('ga_code')}}");
        </script>
    </head>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="d-flex flex-grow text-white">
                            ...
                            <div class="me-3 align-self-center">
                                ...
                                <div>
                                    <a href="{{route('profile-store',['code' => $code])}}" class="text-white">
                                        Setting Profile
                                    </a>
                                </div>
                                @else
                                ...
                                <div>
                                    <a href="{{route('signup-store',['code' => $code])}}"
                        ...
                        <div class="nav mt-4 align-items-end">
                            <ul>
                                <li><a href="{{route('index-store',['code' => $code])}}">Home</a></li>
                                @if (auth()->user())
                                <!-- Register user menu -->
                                <li>
                                    <a href="{{route('my-order-index-store',['code' => $code])}}">My Order
                                        <span class="badge bg-warning float-end">
                                            {{
                                                DB::table('merchant_orders')
                                                ->join('merchants','merchants.id','=','merchant_orders.merchant_id')
                                                ->where('merchant_orders.user_id',Auth::user()->id)
                                ...
                                <li><a href="{{route('about-us-store',['code' => $code])}}">About us</a></li>
                                <li><a href="{{route('pp-store',['code' => $code])}}">Privacy Policy</a></li>
                                <li><a href="{{route('tnc-store',['code' => $code])}}">Term & Condition</a></li>
                                @if (auth()->user())
                                    <li><a href="{{route('logout-store',['code' => $code])}}">Sign Out</a></li>
                                @else
                                    <li><a href="{{route('login-store',['code' => $code])}}">Sign In</a></li>
                                @endif
                            </ul>
                        </div>
...
<script>
    ...
    $('.open-cart').on('click',function () {
        ...
        $.ajax({
            type: "GET",
            url: "{{route('index-shopping-cart-store',['code'=>$code])}}",
            data: {cart:JSON.parse(sessionStorage.getItem('cart-oobe-store'))},
            success: function (response) {
                $.get("{{route('index-shopping-cart-store',['code'=>$code])}}",
