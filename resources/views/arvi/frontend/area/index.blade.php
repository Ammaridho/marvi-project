...
@section('content')
                            ...
                            @if (isset($item))
                                <a href="{{ $item->url }}" class="me-2">
                                    <div class="img-hero bg-light"
                                        style="background-image: url('/arvi/backend-assets/img/banners/{{$item->image_url}}')">
                                    </div>
                                </a>
                            @endif
                    ...
                    @foreach ($merchants as $item)
                        <div class="pb-4">
                            <div class="card mx-1 shadow">
                                <a href="{{route('index-store-order-area', ['code'=>$code,'merchantId' => $item->id])}}"
                                    class="pick-store">
                                    <div class="card-body text-center merchant-logo">
                                        @if (isset($item->image_url))
                                            <img src="/arvi/backend-assets/img/logo/merchants/{{$item->image_url}}" />
                                        @else
                ...
                <div class="tab-line">
                    <ul class="nav nav-tabs" id="myTab" role="tablist">
                        <li class="nav-item" role="presentation">
                            <button class="nav-link active" id="favourite-tab" data-bs-toggle="tab" data-bs-target="#favourite-tab-pane" type="button" role="tab" aria-controls="favourite-tab-pane" aria-selected="true">Popular</button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="new-tab" data-bs-toggle="tab" data-bs-target="#new-tab-pane" type="button" role="tab" aria-controls="new-tab-pane" aria-selected="false">New</button>
                        </li>
                        <li class="nav-item" role="presentation">
                    ...
                    <div class="tab-content" id="myTabContent">
                        <div class="tab-pane fade show active" id="favourite-tab-pane" role="tabpanel" aria-labelledby="favourite-tab" tabindex="0">
                            <!-- favorit item -->
                            <div class="card-menu">
                                <!--
                                    SKLETEON LOADER FOR LIST ITEM
                                ...
                                <div class="card shadow-sm">
                                    <div class="card-body listitem">
                                        <a href="{{route('show-store-order-area',['code'=>$code,'productId' => $item['id'] ])}}">
                                            <div class="d-flex align-items-center">
                                                <div>
                                                    @if (isset($item['url']))
                                                        @if ($item['url'] != 'url')
                                                            <div class="img-wrapper-sm"
                                                                style="background-image: url('/arvi/backend-assets/img/products/brands/{{$item['url']}}');">
                                                            </div>
        ...
        <script>
            ...
            $('#search-merchant, #searchSomething').on('keyup click',function () {
                let search = $(this).val();
                $.get("{{route('search-area',['code'=>$code])}}",{search:search},function (data) {
            ...
            $('.pick-store').on('click',function () {
                ...
                $.get("{{route('index-store-order-area',['code'=>$code])}}",{merchantId:merchantId},function(data) {
